...
class M_program extends CI_Model{
    ...
    function get_program_by_slug($id) {
		$hsl = $this->db->get_where('jsts_programs', ['slug' => $id])->row_array();
		return $hsl;
    }
	function get_post_from_id($id){
		// $hsl = $this->db->get_where('jsts_keywords', ['id_keyword' => $id]);
		$hsl = $this->db->query("SELECT * FROM `jsts_post` WHERE `programs` LIKE '%".$id."%'");
		return $hsl;
	}
	...
	function get_program_list($name, $limit, $start){
		$query = "SELECT * FROM `jsts_post` WHERE `programs` LIKE '%".$name."%' LIMIT ".$start.", ".$limit."";
		$hsl = $this->db->query($query);
		return $hsl;
    }

	function get_program_by_id($id){
		$hsl = $this->db->get_where('jsts_programs', ['id_programs' => $id])->row_array();
		return $hsl;
	}
    ...
    function insert_program($name,$slug) {
		...
		$this->db->insert('jsts_programs', $data);
    }

	function hapus($id) {
		$hsl = $this->db->delete('jsts_programs', array('id_program' => $id));
		return $hsl;
	}
    ...
    function update($id,$tagname,$tagslug) { 
		...
		$this->db->update('jsts_programs', $data);
    }	
