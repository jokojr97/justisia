...
class Auth extends CI_Controller {
	...
	public function index()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$admin = $this->db->get_where('jsts_user', ['email' => $user])->row_array();
			if ($admin){
				$validate_by = 'email';
			}else {
				$validate_by = 'user';
	...
	private function _login($user, $pass, $validate_by)
	{
		if($validate_by == 'email'){
			...
			$admin = $this->db->get_where('jsts_user', ['username' => $user])->row_array();		
		}
		if($admin) {
	...
	public function daftar()
	{
		...
		$this->form_validation->set_rules('email', 'Email', 'required|trim|valid_email|is_unique[jsts_user.email]',[
		...
		if ($this->form_validation->run() == false) {
			...
			$this->db->insert('jsts_user', $data);
			$this->db->insert('jsts_user_token', $user_token);
	...
	public function verify() {
		...
		$user = $this->db->get_where('jsts_user', ['email' =>$email])->row_array();
		if ($user) {
			$user_token = $this->db->get_where('jsts_user_token', ['token' => $token])->row_array();
			if ($user_token) {
				if (time() - $user_token['date_created'] < (60 * 60 * 24)) {
					$this->db->set('is_active', 1);
					$this->db->where('email', $email);
					$this->db->update('jsts_user');
					$this->db->delete('jsts_user_token', ['email' => $email]);
					...
					$this->db->delete('jsts_user', ['email' => $email]);
					$this->db->delete('jsts_user_token', ['email' => $email]);
	...
	public function daftaradmin()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$this->db->insert('jsts_user_admin', $data);
	...
	public function gantipassword(){
		...
		$role = $this->db->get_where('jsts_user_role', ['id' => $role_id])->row_array();
		$user = $this->db->get_where('jsts_user', ['email' => $email])->row_array();
		$dbuser = 'jsts_user';
	...
	public function forgotPassword(){
		...
		if ($this->form_validation->run() == false) {
			...
			$user = $this->db->get_where('jsts_user', ['email' => $email, 'is_active' => 1])->row_array();
			if ($user) {
				$token = substr(md5(mt_rand()), 0, 32);
				$user_token = [
					'email' => $email,
